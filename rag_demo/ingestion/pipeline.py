...
def run_extraction(f, text):
    ...
    manager = json.loads(bedrock_util.call_language_model(prompt).split('```')[1].strip('json'))
    ...
    for chunk in filing_info_chunks:
        ...
        response = json.loads(bedrock_util.call_language_model(prompt).split('```')[1].strip('json'))
